...
public class LoginManager {
    public void onlineLogin(Client client) throws Exception {
        LoginPacket loginPacket=new LoginPacket();

        KeyPair ecdsa256KeyPair = AuthUtils.createKeyPair();//for xbox live, xbox live requests use, ES256, ECDSA256
        client.publicKey = (ECPublicKey) ecdsa256KeyPair.getPublic();
        client.privateKey = (ECPrivateKey) ecdsa256KeyPair.getPrivate();
        ...
        client.publicKey = (ECPublicKey) ecdsa384KeyPair.getPublic();
        client.privateKey = (ECPrivateKey) ecdsa384KeyPair.getPrivate();
         ...
         */
        String chainData = xbox.requestMinecraftChain(xsts, client.publicKey);
        ...
        {
            String publicKeyBase64 = Base64.getEncoder().encodeToString(client.publicKey.getEncoded());
            ...
            String signatureString = AuthUtils.signBytes(dataToSign,client.privateKey);
        ...
        }
        {
            ...
            client.xuid = extraData.getString("XUID");
            client.playerUUID = UUID.fromString(extraData.getString("identity"));
            client.playerName = extraData.getString("displayName");
        }

        loginPacket.setChainData(new AsciiString(chainDataObject.toJSONString().getBytes(StandardCharsets.UTF_8)));
        loginPacket.setSkinData(new AsciiString(this.getSkinData(client)));
        loginPacket.setProtocolVersion(EZ4H.getCommonManager().getBedrockProtocolVersion());
        client.sendPacket(loginPacket);
    }
    public void offlineLogin(Client client) throws Exception {
        LoginPacket loginPacket=new LoginPacket();
        ...
        extraData.put("identity", client.playerUUID.toString());
        extraData.put("displayName", client.playerName);
    ...
    public String getSkinData(Client client) throws Exception{
        String publicKeyBase64 = Base64.getEncoder().encodeToString(client.publicKey.getEncoded());
        ...
        skinData.put("SelfSignedId", client.playerUUID.toString());//erm? i hope this works?
        ...
        skinData.put("SkinData",getSkinData(client.playerName));
        ...
        skinData.put("SkinId", client.playerUUID.toString()+ ".Custom");//ok..? :shrug:
        ...
        skinData.put("ThirdPartyName", client.playerName);
    ...
    private String getSkinData(String playerName){
